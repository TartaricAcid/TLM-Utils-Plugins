<template>
    <div>
        <div v-if="newModelPack">
            <div style="background-color: #21252b; width: 100%; height: 330px; overflow-y: auto; padding: 10px 20px">
                <div>
                    <h5 style="margin: 0; padding: 0">{{tl("dialog.tlm_utils.load_pack.new.id")}}
                        <span style="color: #ff0000">*</span></h5>
                    <p style="color: #6a6a6d">{{tl("dialog.tlm_utils.load_pack.new.id.desc")}}</p>
                    <input style="border-radius: 1px; margin-top:5px; padding: 5px; width: 100%; height:50px; font-size: 30px; background-color: #1c2026;"
                           v-model="newModelPackId" type="text" @blur="checkNewModelPackId" required>
                </div>
                <div style="margin-top: 10px; height: 20px">
                    <p style="color: red">{{newModelPackIdTip}}</p>
                </div>
                <div style="display: flex; margin-top: 15px">
                    <div style="width: 50%; height: 30px">
                        <button @click="newModelPackIdConfirm" style="width: 98%">
                            {{tl("button.tlm_utils.confirm")}}
                        </button>
                    </div>
                    <div style="width: 50%; height: 30px">
                        <button @click="newModelPackIdCancel" style="width: 98%; margin-left: 2%">
                    ...
                    <div v-if="isEditPackInfo" style="height: 100%">
                        <div style="background-color: #21252b; width: 100%; height: 330px; overflow-y: auto; padding: 10px 20px">
                            ...
                            <div style="display: flex; align-items: center; margin-top: 20px">
                                ...
                                <div style="margin-left: 20px">
                                    <h5 style="margin: 0; padding: 0">
                                        {{tl("dialog.tlm_utils.load_pack.edit.icon")}}</h5>
                            ...
                            <div style="display: flex; align-items: center; margin-top: 20px">
                                ...
                                <div style="margin-left: 20px">
                                    <p style="margin: 0; padding: 0; font-size: large">
                                        {{tl("dialog.tlm_utils.load_pack.edit.version")}}</p>
                                    <p style="margin: 0; padding: 0; color: #6a6a6d">
                            ...
                            <div style="display: flex; align-items: center; margin-top: 20px">
                                ...
                                <div style="margin-left: 20px">
                                    <p style="margin: 0; padding: 0; font-size: large">
                                        {{tl("dialog.tlm_utils.load_pack.edit.author")}}</p>
                                    <p style="margin: 0; padding: 0; color: #6a6a6d">
                                        {{tl("dialog.tlm_utils.load_pack.edit.author.desc")}}</p>
                                </div>
                            </div>
                            <div style="margin-top: 20px">
                                <p style="margin: 0; padding: 0; font-size: large">
                                    {{tl("dialog.tlm_utils.load_pack.edit.description")}}</p>
                                <p style="margin: 0; padding: 0; color: #6a6a6d">
                        ...
                        <div style="display: flex; margin: 10px">
                            <div style="width: 50%">
                                <button style="width: 98%"
                                        @click="clickConfirm">{{tl("button.tlm_utils.confirm")}}
                                </button>
                            </div>
                            <div style="width: 50%">
                                <button style="width: 98%; margin-left: 2%"
                                        @click="clickCancel">{{tl("button.tlm_utils.cancel")}}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div v-if="isShowList">
                    <ul style="max-height: 550px; overflow-y: auto; text-align: center;">
                        <li v-for="modelInfo in showInfo.data['model_list']" :key="modelInfo['model_id']">
                            <button style="width: 98%; height: 30px; margin: 1px; font-size: small">
        ...
        methods: {
            ...
            getIconPath: function (packInfo) {
                ...
                if (packInfo && packInfo.data && packInfo.data["icon"]) {
                    ...
                    let iconPath = `${this.assetsPath}/${icon}`;
            ...
            checkNewModelPackId: function () {
                ...
                if (this.packEditDialog.sidebar.pages[this.newModelPackId]) {
            ...
            newModelPackIdConfirm: function () {
                let pages = this.packEditDialog.sidebar.pages;
                if (pages && this.checkNewModelPackId()) {
                    let createRootPath = `${this.assetsPath}/${this.newModelPackId}`;
                    ...
                    pages[this.newModelPackId] = this.newModelPackId;
                    this.packEditDialog.sidebar.setPage(this.newModelPackId);
                    this.packEditDialog.sidebar.build();
                }
            },
            newModelPackIdCancel: function () {
                let pages = this.packEditDialog.sidebar.pages;
                if (pages && Object.keys(pages).length > 0) {
                    this.packEditDialog.sidebar.setPage(Object.keys(pages)[0]);
                } else {
                    this.packEditDialog.sidebar.setPage("");
                }
            },
