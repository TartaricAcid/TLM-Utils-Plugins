<template>
    <div>
        <div class="data-main">
            ...
            <vue-prism-editor :highlight="highlighter" :line-numbers="false" class="js_input dark_bordered" v-if="!collapse" v-model="example"/>
            <div style="display: flex; margin-top: 10px; font-size: larger">
                <button :title="tl('dialog.tlm_utils.add_present.custom.button.import')" @click="importCode" style="width: 5%; min-width: 5%; padding: 0; margin-left: 83%">
                    <i class="fas fa-file-import fa-fw"></i></button>
                <button :title="tl('dialog.tlm_utils.add_present.custom.button.save')" @click="saveCode" style="width: 5%; min-width: 5%; padding: 0; margin-left: 1%">
                    <i class="fas fa-save fa-fw"></i></button>
                <button :title="tl('dialog.tlm_utils.add_present.custom.button.clear')" @click="clearCode" style="width: 5%; min-width: 5%; padding: 0; margin-left: 1%">
                    <i class="fas fa-trash-alt fa-fw"></i></button>
            </div>
            <vue-prism-editor :highlight="highlighter" :line-numbers="true" @blur="cacheCode" class="js_input dark_bordered" v-model="code"/>
            <vue-prism-editor :highlight="highlighterError" :line-numbers="false" class="error_output" readonly="true" v-if="error" v-model="error"/>
